...
def generate_sampled_graph_and_labels(triplets, sample_size, split_size,
    ...
    logging.info("# sampled edges: {}".format(len(src) * 2))
    g, src_o, rel, norm= build_graph_from_triplets(len(uniq_v), num_rels,(src, rel, dst))
    return g, uniq_v, src_o, rel, norm, samples, labels
...
def build_graph_from_triplets(num_nodes, num_rels, triplets):
    ...
    # adding self-loop
    #src_self = np.arange(num_nodes)
    #rel_self = (np.zeros(num_nodes)+num_rels*2).astype('int64')
    #src, dst = np.concatenate((src, src_self)), np.concatenate((dst, src_self))
    #rel = np.concatenate((rel, rel_self))
    ...
    '''sorted with rel'''
    edges = sorted(zip(rel, dst, src))
    rel, dst, src = np.array(edges).transpose()

    g.add_edges(src, dst)
    norm = comp_deg_norm(g)
    logging.info("# nodes: {}, # edges: {}".format(num_nodes, len(src)))
    return g, src.astype('int64'), rel.astype('int64'), norm
    #return g, src.astype('int64'), rel.astype('int64')

def build_graph_from_triplets_ori(num_nodes, num_rels, triplets):
    """ Create a DGL graph. The graph is bidirectional because RGCN authors
        use reversed relations.
        This function also generates edge type and normalization factor
        (reciprocal of node incoming degree)
    ...
    src, dst = np.concatenate((src, dst)), np.concatenate((dst, src))
    rel = np.concatenate((rel, rel + num_rels))
    # sorted edge with dst
    #edges = sorted(zip(dst, src, rel))
    #dst, src, rel = np.array(edges).transpose()

    # sorted edge with rel
    edges = sorted(zip(rel, dst, src))
    rel, dst, src = np.array(edges).transpose()
...
def build_test_graph(num_nodes, num_rels, edges):
    ...
    return build_graph_from_triplets_ori(num_nodes, num_rels, (src, rel, dst))
...
def perturb_and_get_raw_rank(embedding, w, a, r, b, test_size, batch_size=100):
    ...
    for idx in range(n_batch):
        logging.info("batch {} / {}".format(idx, n_batch))
...
def calc_raw_mrr(embedding, w, test_triplets, hits=[], eval_bz=100):
    with torch.no_grad():
        ...
        logging.info("MRR (raw): {:.6f}".format(mrr.item()))

        hit_k = []
        for hit in hits:
            avg_count = torch.mean((ranks <= hit).float())
            logging.info("Hits (raw) @ {}: {:.6f}".format(hit, avg_count.item()))
            hit_k.append(avg_count.item())
    return mrr.item()
...
def perturb_o_and_get_filtered_rank(embedding, w, s, r, o, test_size, triplets_to_filter):
    ...
    for idx in range(test_size):
        #if idx % 5000 == 0:
        #logging.info("test triplet {} / {}".format(idx, test_size))
        target_s = s[idx]
        target_r = r[idx]
        target_o = o[idx]
...
def calc_filtered_mrr(embedding, w, train_triplets, valid_triplets, test_triplets, hits=[]):
    with torch.no_grad():
        ...
        mrr = torch.mean(1.0 / ranks.float())
        logging.info("MRR (filtered): {:.6f}".format(mrr.item()))

        hit_k = []
        for hit in hits:
            avg_count = torch.mean((ranks <= hit).float())
            logging.info("Hits (filtered) @ {}: {:.6f}".format(hit, avg_count.item()))
            hit_k.append(avg_count.item())
    return mrr.item(), hit_k
...
def calc_mrr(embedding, w, train_triplets, valid_triplets, test_triplets, hits=[], eval_bz=100, eval_p="filtered"):
    if eval_p == "filtered":
        mrr, hit_k = calc_filtered_mrr(embedding, w, train_triplets, valid_triplets, test_triplets, hits)
    else:
        mrr, hit_k = calc_raw_mrr(embedding, w, test_triplets, hits, eval_bz)
    return mrr, hit_k
